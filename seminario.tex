...
\belowdisplayshortskip=0pt	
 ...
 \section{Programação Reflexiva}
	 \begin{frame}{O que é reflexão}
	 	\begin{block}{Definição}
	 		\alert{Escrever --  Fazendo ponte com a seção anterior (ver uma maneira legal).}
	 ...
	 \begin{frame}{História}
	 	Colocar a história.
  	 \end{frame}
	 \begin{frame}{Reificação}
	 	\begin{block}{Definições}
	 		''The treatment of something abstract as a material or concrete thing'' (Enciclopédia britanica)
	 		\\ Reificação é a conversão de um componente de um programa em algo por ele manipulável.  \cite{friedman1984reification}
	 	\end{block}	 	 			
	 	\begin{block}{ }
		 	\alert{Figura reificação}
	 	\end{block}	 	 			
	 \end{frame}	  	 
